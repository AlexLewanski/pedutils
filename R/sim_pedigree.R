...
################################################
### ASSORTED FUNCTIONS RELATED TO SIMULATION ###
################################################

#' Convert pedigree dataframe generated from sim_ped_repeat_attempt to pedsim2relateStats format and optionally export
#'
#' @param ped_output pedigree dataframe in format generated by sim_ped_repeat_attempt
#' @param export logical specifying whether the formatted pedigree should be exported
#' @param export_dir directory for where the file should be exported (if export_dir = TRUE)
...
pedsim2relateStats <- function(ped_output,
  ...
  processed_df <- ped_output %>%
    ...
    dplyr::mutate(SEX = dplyr::case_when(.data$SEX == 'M' ~ 'MALE',
                                         .data$SEX == 'F' ~ 'FEMALE',
                                         TRUE ~ '*'),
           SIRE_ID = dplyr::if_else(.data$SIRE_ID == '0', '*', .data$SIRE_ID),
           DAM_ID = dplyr::if_else(.data$DAM_ID == '0', '*', .data$DAM_ID),
           COHORT = dplyr::if_else(is.na(.data$COHORT), '*', as.character(.data$COHORT)),
           COHORT_LAST = dplyr::if_else(is.na(.data$COHORT_LAST), '*', as.character(.data$COHORT_LAST)))

  if (isTRUE(export)) {
    utils::write.table(x = processed_df,
                       file = paste0(export_dir, '/', export_file_name),
                       quote = FALSE,
                       sep = "\t",
                       row.names = FALSE,
                       col.names = TRUE)
